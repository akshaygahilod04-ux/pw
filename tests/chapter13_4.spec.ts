...
test("Automate Compound Interest Calculator", async ({ page }) => {
  ...
  const interestBlock:Locator = page.locator("div.text-s-24.font-bold.text-blue-350").nth(1);
  ...
  const amountSpan:Locator = interestBlock.locator("span").nth(1);
  const amountText:string|undefined = (await amountSpan.textContent())?.trim();
  ...
  const totalBlock:Locator = page.locator("div.text-s-24.font-bold.text-blue-350").nth(2);

  //Assert on the locator (auto-waits)
  await expect(totalBlock).toContainText("7,700");

  //Print the actual value (the number is usually in the 2nd <span>)
  const totalamountSpan = totalBlock.locator("span").nth(1);
  const totalamountText = (await totalamountSpan.textContent())?.trim();
  console.log("total amount:", totalamountText); // should print "700"
